...
namespace Salad {
	...
	public:
		...
		Ref<TextureMap> loadTextureMap(const std::string& filepath, uint32_t gridCountH, uint32_t gridCountV);

		Ref<Texture2D> getTexture2D(const std::string& textureId);
		Ref<TextureMap> getTextureMap(const std::string& textureMap);
	...
	private:
		...
		std::unordered_map<std::string, Ref<TextureMap>> m_TextureMapCollection;
	};
