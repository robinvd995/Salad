...
namespace Salad {
	...
	void EntityComponentPlayerController::onComponentUpdate(Timestep ts) {
		...
		glm::vec3& position = m_Transform->getTransform()->getPosition();
		...
		if (moving) {
			...
			m_Transform->getTransform()->translate(normalizedVector * speed);
		}
	...
	// -------- Entity Behaviour Start --------

	void EntityComponentBehaviour::onComponentAttached() {
		Ref<EntityComponentTransform> transformComponent = m_Entity.lock()->getComponent<EntityComponentTransform>();
		m_Behaviour->setTransform(transformComponent->getTransform());

		m_Behaviour->start();
	}

	void EntityComponentBehaviour::onComponentUpdate(Timestep ts) {
		m_Behaviour->update(ts);
	}
